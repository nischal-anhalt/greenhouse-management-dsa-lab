...
For our curiosity extension, let's implement Role-Based Authorization (RBAC).

Right now, anyone with a valid Keycloak token can do anything in our system. We are going to lock down the application so that only users who explicitly possess the ROLE_USER role are allowed to create (POST) new items.

1. Modify `create_item` function for `POST /items` at `greenhouse-service-lab-1/app.py` and rebuild the REST service. `docker compose up -d --build rest-service`
...
def create_item():
    ...
    # ...
```
2. Get auth token from two users with different roles. `alice --> ROLE_USER` and `bob --> ROLE_GUEST`
...
curl -s \
  ...
  -d "username=bob" \
  -d "password=secret" \
  "${KC}/token" | jq -r .access_token > token-bob.jwt
```
```bash
KC="http://localhost:8088/realms/dsa-lab/protocol/openid-connect"
...
curl -s \
  ...
  -d "username=alice" \
  -d "password=secret" \
  "${KC}/token" | jq -r .access_token > token-alice.jwt
```

3. Test the authorization

Create request With `ROLE_GUEST`
```bash
curl -i --cacert security/certs/ca.crt \
  -H "Authorization: Bearer $(cat token-bob.jwt)" \
  -H "Content-Type: application/json" \
  -d '{"name": "RBAC Test Plant", "status": "healthy", "location": "Zone 1"}' \
  https://localhost:8443/api/items
```
```bash
HTTP/2 403 
content-type: application/json
date: Tue, 04 Aug 2026 12:56:10 GMT
server: Werkzeug/3.1.8 Python/3.12.13
content-length: 93
...
Create request with `ROLE_USER`
```bash
curl -i --cacert security/certs/ca.crt \
  -H "Authorization: Bearer $(cat token-alice.jwt)" \
  -H "Content-Type: application/json" \
  -d '{"name": "RBAC Test Plant", "status": "healthy", "location": "Zone 1"}' \
  https://localhost:8443/api/items
```
...
HTTP/2 201 
content-type: application/json
date: Tue, 04 Aug 2026 12:56:23 GMT
server: Werkzeug/3.1.8 Python/3.12.13
content-length: 69
